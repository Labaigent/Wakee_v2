...
interface Props {
  señal: SenalMercado;
  isExpanded: boolean;
  /** No ID argument needed — the parent pre-binds the señal ID before passing this down. */
  onToggle: () => void;
}
...
/**
 * Displays a single market señal as an expandable card.
 * Expansion state is controlled externally — this component is purely presentational.
 */
export function SenalMercadoCard({ señal, isExpanded, onToggle }: Props) {
  // Boolean() makes the intent explicit: we only care whether content exists, not its value.
  const hasExpandedContent = Boolean(señal.detalles || señal.hipotesis_inmobiliaria);
    ...
    <div className="border-2 border-[#DCDEDC] rounded-lg p-4 sm:p-5 hover:border-[#1F554A] transition-colors">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <h4 className="font-medium text-base text-[#141414]">{señal.titulo}</h4>
            {señal.ultima_fecha_actualizacion && (
              <Badge variant="secondary" className="text-xs bg-[#DCDEDC] text-gray-700">
                {formatDate(señal.ultima_fecha_actualizacion)}
              </Badge>
            )}
          </div>
          {señal.subtitulo && (
            <p className="text-sm font-medium text-[#1F554A] mb-2">{señal.subtitulo}</p>
          )}
        </div>
      ...
      {isExpanded && (
        <div className="mb-3 space-y-2">
          {señal.detalles && (
            <p className="text-sm text-gray-700 leading-relaxed">{señal.detalles}</p>
          )}
          {señal.hipotesis_inmobiliaria && (
            <p className="text-sm text-gray-600 leading-relaxed italic">{señal.hipotesis_inmobiliaria}</p>
          )}
        </div>
      )}
